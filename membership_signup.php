<?php
	...
	if($_POST['signUp'] != ''){
		...
		// validate data
		if($memberID == ''){
			echo error_message($Translation['username empty']);
			exit;
		}
		...
		if(sqlValue("select count(1) from membership_users where lcase(memberID)='$memberID'")){
			echo error_message($Translation['username exists']);
			exit;
		}
		if(!$email){
				...
				<div class="panel-body">
					<form method="post" action="membership_signup.php" onSubmit="return jsValidateSignup();">
						<div class="form-group">
							...
							<span id="usernameNotAvailable" class="help-block invisible"><i class="glyphicon glyphicon-remove"></i> <?php echo str_ireplace(array("'", '"', '<memberid>'), '', $Translation['username exists']); ?></span>
						</div>
	...
	<script>
		document.observe("dom:loaded", function() {
			...
			jQuery('#password').bind('keyup blur', function(){
				...
				if(ps == 'strong'){
					jQuery('#password').parent().removeClass('has-error has-warning').addClass('has-success');
					jQuery('#password').attr('title', '<?php echo htmlspecialchars($Translation['Password strength: strong']); ?>');
				}else if(ps == 'good'){
					jQuery('#password').parent().removeClass('has-success has-error').addClass('has-warning');
					jQuery('#password').attr('title', '<?php echo htmlspecialchars($Translation['Password strength: good']); ?>');
				}else{
					jQuery('#password').parent().removeClass('has-success has-warning').addClass('has-error');
