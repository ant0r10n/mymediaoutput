	...
	function Render(){
		...
		if($this->ListType != 2){
			if($this->ApplySelect2){
				$this->HTML .= "<select tabindex=\"1\" style=\"width: 100%;\" name=\"$this->SelectName".($this->ListType==3 ? '[]' : '')."\" id=\"$this->SelectID\"".($this->ListType==1 ? ' size="'.($this->ListBoxHeight < $ArrayCount ? $this->ListBoxHeight : ($ArrayCount + ($this->AllowNull ? 1 : 0))).'"' : '').($this->ListType==3 ? ' multiple' : '').'>';
			}else{
			...
			$this->HTML .= '</select>';
			if($this->ApplySelect2){
				$this->HTML .= '<script>jQuery(function(){ jQuery("#' . $this->SelectID . '").select2({ minimumResultsForSearch: 15 }); })</script>';
			}
