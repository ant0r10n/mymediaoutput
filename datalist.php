...
class DataList{
	...
	// disable multi-selection if too many records to avoid browser performance issues
		if($RecordCount > datalist_max_records_multi_selection) $this->AllowPrintingMultiSelection = 0;
	...
	// should SelectedID be reset on clicking TV buttons?
		...
		if($current_view == 'DV'){
			$this->HTML .= '<div class="page-header">';
				$this->HTML .= '<h1>';
					$this->HTML .= '<img src="' . $this->TableIcon . '"> ' . $this->TableTitle;
				$this->HTML .= '</h1>';
			$this->HTML .= '</div>';
		}
	...
	// quick search and TV action buttons  
		if(!$this->HideTableView && !($dvprint_x && $this->AllowSelection && $SelectedID) && !$PrintDV){
			$buttons_html = $quick_search_html = '';

			if($Print_x == ''){

				// display 'Add New' icon
				if($this->Permissions[1] && $this->SeparateDV){
					$buttons_html .= '<button type="submit" id="addNew" name="addNew_x" value="1" class="btn btn-success"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Add New'] . '</button>';
					$buttonsCount++;
				}

				// display Print icon
				if($this->AllowPrinting){
					$buttons_html .= '<button onClick="document.myform.NoDV.value=1; ' . $resetSelection . ' return true;" type="submit" name="Print_x" id="Print" value="1" class="btn btn-default"><i class="glyphicon glyphicon-print"></i> ' . $Translation['Print Preview'] . '</button>';
					$buttonsCount++;
				}

				// display CSV icon
				if($this->AllowCSV){
					$buttons_html .= '<button onClick="document.myform.NoDV.value=1; ' . $resetSelection . ' return true;" type="submit" name="CSV_x" id="CSV" value="1" class="btn btn-default"><i class="glyphicon glyphicon-download-alt"></i> ' . $Translation['CSV'] . '</button>';
					$buttonsCount++;
				}

				// display Filter icon
				if($this->AllowFilters){
					$buttons_html .= '<button onClick="document.myform.NoDV.value=1; ' . $resetSelection . ' return true;" type="submit" name="Filter_x" id="Filter" value="1" class="btn btn-default"><i class="glyphicon glyphicon-filter"></i> ' . $Translation['filter'] . '</button>';
					$buttonsCount++;
				}

				// display Show All icon
				if(($this->AllowFilters)){
					$buttons_html .= '<button onClick="document.myform.NoDV.value=1; ' . $resetSelection . ' return true;" type="submit" name="NoFilter_x" id="NoFilter" value="1" class="btn btn-default"><i class="glyphicon glyphicon-remove-circle"></i> ' . $Translation['Reset Filters'] . '</button>';
					$buttonsCount++;
				}

				$quick_search_html .= '<div class="input-group" id="quick-search">';
					$quick_search_html .= '<input tabindex="1" type="text" name="SearchString" value="' . htmlspecialchars($SearchString, ENT_QUOTES) . '" class="form-control" placeholder="' . htmlspecialchars($this->QuickSearchText) . '">';
					$quick_search_html .= '<span class="input-group-btn">';
						$quick_search_html .= '<button tabindex="2" name="Search_x" value="1" id="Search" type="submit" onClick="' . $resetSelection . ' document.myform.NoDV.value=1; return true;"  class="btn btn-default" title="' . htmlspecialchars($this->QuickSearchText) . '"><i class="glyphicon glyphicon-search"></i></button>';
						$quick_search_html .= '<button tabindex="3" name="NoFilter_x" value="1" id="NoFilter_x" type="submit" onClick="' . $resetSelection . ' document.myform.NoDV.value=1; return true;"  class="btn btn-default" title="' . htmlspecialchars($Translation['Reset Filters']) . '"><i class="glyphicon glyphicon-remove-circle"></i></button>';
			...
			$this->HTML .= '<div class="page-header">';
				...
				$this->HTML .= '</h1>';
			$this->HTML .= '</div>';
			$this->HTML .= '<div id="topButtons">';
				$this->HTML .= '<div class="btn-group btn-group-lg visible-md visible-lg">' . $buttons_html . '</div>';
				$this->HTML .= '<div class="btn-group-vertical btn-group-lg visible-xs visible-sm">' . $buttons_html . '</div>';
			...
			if($Print_x){
				if($this->AllowPrintingMultiSelection){
					$withSelected = ''.
						'<div class="btn-group hidden btn-group-lg" id="withSelected">' .
							'<button class="btn btn-default" type="submit" name="PrintTV" value="1"><i class="glyphicon glyphicon-print"></i> ' . $Translation['Print Preview Table View'] . '</button>' .
							($AllowPrintDV ? '<button id="PrintDV" class="btn btn-default" type="submit" name="PrintDV" value="1"><i class="glyphicon glyphicon-print"></i> ' . $Translation['Print Preview Detail View'] . '</button>' : '') .
							'<button class="btn btn-default" type="submit" name="Print_x" value="1"><i class="glyphicon glyphicon-remove-circle"></i> ' . $Translation['Cancel Selection'] . '</button>'.
						'</div>' .
						'<script>'.
							'var countSelected = 0; '.
							'jQuery(function(){'.
								'setInterval("'.
									'if(countSelected){ jQuery(\'#withSelected\').removeClass(\'hidden\'); }else{ jQuery(\'#withSelected\').addClass(\'hidden\'); }'.
								'", 500); '.
							'});'.
						'</script>';
				}
				...
				/* action buttons */
				$this->HTML .= '<div class="hidden-print">';
					$this->HTML .= '<div class="btn-toolbar">';
						$this->HTML .= '<div class="btn-group btn-group-lg">';
							$this->HTML .= '<button class="btn btn-primary" type="button" id="sendToPrinter" onClick="window.print();"><i class="glyphicon glyphicon-print"></i> ' . $Translation['Print'] . '</button>';
						$this->HTML .= '</div>';
						$this->HTML .= '<div class="btn-group btn-group-lg">';
							$this->HTML .= '<button class="btn btn-warning" type="submit"><i class="glyphicon glyphicon-remove-circle"></i> ' . $Translation['Cancel Printing'] . '</button>';
						$this->HTML .= '</div>';
						$this->HTML .= $withSelected;
					$this->HTML .= '</div>';
					$this->HTML .= '<div class="clearfix"></div><p></p>';
				$this->HTML .= '</div>';
			}
			...
			$this->HTML .= '<script>document.observe("dom:loaded", function() { if($$("input[name=SearchString]")){ $$("input[name=SearchString]")[0].focus(); } });</script>';
	...
	// begin table and display table title
		if(!$this->HideTableView && !($dvprint_x && $this->AllowSelection && $SelectedID) && !$PrintDV && !$Embedded){
			...
			$this->HTML .= '<thead><tr>';
			if($Print_x == '' || !$this->AllowPrintingMultiSelection){ $this->HTML .= '<th style="width: 18px;">'.($this->AllowSelection ? '&nbsp;' : '')."</th>"; }
			if($this->AllowPrintingMultiSelection && $Print_x!='') $this->HTML .= '<th style="width: 18px;" class="text-center"><input class="hidden-print" type="checkbox" title="'.htmlspecialchars($Translation['Select all records']).'" id="toggleAll" onclick="toggleAllRecords();"></th>';
		// Templates
			if($this->Template!=''){
		...
		// end of table view navigation code
			...
			// execute query for table view
				...
				while(($row = mysql_fetch_array($result)) && ($i < ($FirstRecord + $this->RecordsPerPage))){
					$alt=(($i-$FirstRecord)%2);
					if($PrintTV && $_POST["select_{$row[$FieldCountTV]}"]!=1)    continue;
					$class = "TableBody".($alt ? 'Selected' : '').($fNumeric ? 'Numeric' : '');

					if($Print_x == '' || !$this->AllowPrintingMultiSelection){ $this->HTML .= ($SelectedID == $row[$FieldCountTV] ? '<tr class="active"><td class="text-center"><i class="glyphicon glyphicon-chevron-right"></i></td>' : '<tr><td class="text-center">&nbsp;</td>'); }
					if($this->AllowPrintingMultiSelection && $Print_x!=''){
						$this->HTML .= "<tr><td class=\"text-center\"><input class=\"hidden-print\" type=\"checkbox\" id=\"select_{$row[$FieldCountTV]}\" name=\"select_{$row[$FieldCountTV]}\" value=\"1\" onclick=\"if(\$('select_{$row[$FieldCountTV]}').checked) countSelected++; else countSelected--;\"></td>";
	...
	// display details form ...
		if(($this->AllowSelection || $this->AllowInsert || $this->AllowUpdate || $this->AllowDelete) && $Print_x=='' && !$PrintDV){
			if(($this->SeparateDV && $this->HideTableView) || !$this->SeparateDV){
				...
				$this->HTML .= "\n\t<div class=\"panel panel-default\">{$dvCode}</div>";
	...
	// display multiple printable detail views
		if($PrintDV){
			...
			if($selectedRecords <= datalist_max_records_dv_print){ // if records selected > {datalist_max_records_dv_print} don't show DV preview to avoid db performance issues.
				foreach($_POST as $n => $v){
					if(strpos($n, 'select_') === 0){
						$id=str_replace('select_', '', $n);
